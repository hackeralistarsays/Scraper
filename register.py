...
class Register:
    def __init__(self, root):
        ...
        #==Left Image==
        self.left=ImageTk.PhotoImage(file='images/dep.jpg')
        ...
        btn_login = Button(self.root, text="Sign In", font=('times new roman', 20), bg='white', bd=0, cursor='hand2').place(x=200, y=490, width=150)
    ...
    def register_data(self):
        if self.txt_fname.get() == '' or self.txt_lname.get() == '' or self.txt_contact.get() == '' or self.txt_email.get() == '' or self.cmb_quest.get() == 'Select' or self.txt_answer.get() == '' or self.txt_password.get() == '' or self.txt_cpassword.get() == '':
            messagebox.showerror('Required Fields', 'All Fields Are Required!!', parent=self.root)

        elif self.txt_password.get() != self.txt_cpassword.get():
            messagebox.showerror('Password Missmatch', 'Your Password And Confirm Password Do NOT MATCH', parent=self.root)

        elif self.var_chk.get() == 0:
            messagebox.showerror('Terms Agreement', 'Please Agree To The Terms and Conditions', parent=self.root)
        else:
            try:

                db.insert(self.txt_fname.get(), self.txt_lname.get(), self.txt_contact.get(), self.txt_email.get(), self.cmb_quest.get(),
              self.txt_answer.get(), self.txt_password.get())
                ...
                clear_text()
                messagebox.showinfo('Success', 'Your Registration was Successful', parent=self.root)  


     

            except Exception as es:
                messagebox.showerror('Error', f'Your Registration was not Successful due to: {str(es)}', parent=self.root)      
